...
{
    if (lastColWatchPosition)
        delete lastColWatchPosition;
    if (colWatch)
        delete colWatch;
    if (nearby)
        delete nearby;
...
void CVX_Voxel::timeStep(float dt)
{
    ...
    if (isDragEnabled())
    {
        ...
        double fluid_rho = 1000;  // density of water Kg/m^3
        double drag_coef = 1.05;  // drag coefecient of a cube https://en.wikipedia.org/wiki/Drag_coefficient
        double x = mat->size().x; // x length of voxel cube
        double y = mat->size().y; // y length of voxel cube
        double z = mat->size().z; // z length of voxel cube
        double area = 0;          // area of face in direction
        Vec3D<double> p1;
        Vec3D<double> p2;
        Vec3D<double> p3;
        ...
        {
            if (TAG == "CORE" && ID == 2 && dir == 4)
            {
                continue;
            }
            if (TAG == "CORE" && ID == 3 && dir == 5)
            {
                continue;
            }
            switch (CVX_Voxel::linkDirection(dir))
            {
            case X_POS:
                area = y * z;
                p1 = (Vec3D<double>)cornerPosition(PPP);
                p2 = (Vec3D<double>)cornerPosition(PPN);
                p3 = (Vec3D<double>)cornerPosition(PNP);
                break;
            case X_NEG:
                area = y * z;
                p1 = (Vec3D<double>)cornerPosition(PPP);
                p2 = (Vec3D<double>)cornerPosition(PNP);
                p3 = (Vec3D<double>)cornerPosition(PPN);
                break;
            case Y_POS:
                area = x * z;
                p1 = (Vec3D<double>)cornerPosition(PPP);
                p2 = (Vec3D<double>)cornerPosition(NPP);
                p3 = (Vec3D<double>)cornerPosition(NPN);
                break;
            case Y_NEG:
                area = x * z;
                p1 = (Vec3D<double>)cornerPosition(PPP);
                p2 = (Vec3D<double>)cornerPosition(NPN);
                p3 = (Vec3D<double>)cornerPosition(NPP);
                break;
            case Z_POS:
                area = x * y;
                p1 = (Vec3D<double>)cornerPosition(PPP);
                p2 = (Vec3D<double>)cornerPosition(PNP);
                p3 = (Vec3D<double>)cornerPosition(NNP);
                break;
            case Z_NEG:
                area = x * y;
                p1 = (Vec3D<double>)cornerPosition(PPP);
                p2 = (Vec3D<double>)cornerPosition(NNP);
                p3 = (Vec3D<double>)cornerPosition(PNP);
                break;
            default:
                NULL;
            }
            if (adjacentVoxel(CVX_Voxel::linkDirection(dir)) == NULL)
            {
                Vec3D<double> linV = linMom / mat->mass(); // velocity = momenmtum / mass of voxel
                Vec3D<double> norm_vec = (p3 - p1).Cross(p2 - p1);
                norm_vec.NormalizeFast();
                linV.Dot(norm_vec);
                linV.NormalizeFast(linV.Length2());
                Vec3D<double> Fd = -(1.0 / 2.0) * fluid_rho * drag_coef * area * linV;
                curForce += Fd;
            }
        }
    }
    ...
    if (isFloorEnabled())
        floorForce(
            dt, &curForce); // floor force needs dt to calculate threshold to "stop" a slow voxel into static friction.

    fricForce = curForce - fricForce;

    assert(!(curForce.x != curForce.x) || !(curForce.y != curForce.y) || !(curForce.z != curForce.z)); // assert non
                                                                                                       // QNAN
    linMom += curForce * dt;
    ...
    { // we must catch a slowing voxel here since it all boils down to needing access to the dt of this timestep.
        ...
        if (hKe + work <= 0)
            setFloorStaticFriction(true); // this checks for a change of direction according to the work-energy
                                          // principle
    ...
    if (ext)
    {
        ...
        if (ext->isFixedAnyRotation())
        { // if any rotation fixed, all are fixed
...
Vec3D<double> CVX_Voxel::force()
{
    ...
    assert(!(totalForce.x != totalForce.x) || !(totalForce.y != totalForce.y) ||
           !(totalForce.z != totalForce.z)); // assert non QNAN
...
void CVX_Voxel::floorForce(float dt, Vec3D<double> *pTotalForce)
{
    ...
    if (CurPenetration >= 0)
    {
        ...
        pTotalForce->z +=
            normalForce - mat->collisionDampingTranslateC() * vel.z; // in the z direction: k*x-C*v - spring and damping

        if (isFloorStaticFriction())
        { // If this voxel is currently in static friction mode (no lateral motion)
            assert(horizontalVel.Length2() == 0);
            float surfaceForceSq = (float)(pTotalForce->x * pTotalForce->x +
                                           pTotalForce->y * pTotalForce->y); // use squares to avoid a square root
            float frictionForceSq = (mat->muStatic * normalForce) * (mat->muStatic * normalForce);

            if (surfaceForceSq > frictionForceSq)
                setFloorStaticFriction(false); // if we're breaking static friction, leave the forces as they currently
                                               // have been calculated to initiate motion this time step
        }
        else
        { // even if we just transitioned don't process here or else with a complete lack of momentum it'll just go back
          // to static friction
            *pTotalForce -= mat->muKinetic * normalForce *
                            horizontalVel.Normalized(); // add a friction force opposing velocity according to the
                                                        // normal force and the kinetic coefficient of friction
        }
    }
    else
        setFloorStaticFriction(false);
...
Vec3D<float> CVX_Voxel::strain(bool poissonsStrain) const
{
    ...
    // if one connection in positive or negative direction of a particular axis, strain is that strain - ?? and force or
    // constraint? if connections in both the positive and negative directions of a particular axis, strain is the
    // average.
...
float CVX_Voxel::transverseStrainSum(CVX_Link::linkAxis axis)
{
    ...
    switch (axis)
    {
    case CVX_Link::X_AXIS:
        return psVec.y + psVec.z;
    case CVX_Link::Y_AXIS:
        return psVec.x + psVec.z;
    case CVX_Link::Z_AXIS:
...
void CVX_Voxel::enableCollisions(bool enabled, float watchRadius)
{
    ...
    {
        if (!lastColWatchPosition)
            lastColWatchPosition = new Vec3D<float>;
        if (!colWatch)
            colWatch = new std::vector<CVX_Collision *>;
        if (!nearby)
            nearby = new std::vector<CVX_Voxel *>;
    }
    ...
    for (int k = 0; k < linkDepth; k++)
    {
        ...
        while (iCurrent != iPassEnd)
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                if (pV2 && std::find(allNearby.begin(), allNearby.end(), pV2) == allNearby.end())
                    allNearby.push_back(pV2);
            }
        }
    }
