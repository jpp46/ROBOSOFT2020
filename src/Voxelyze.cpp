...
CVoxelyze &CVoxelyze::operator=(CVoxelyze &VIn)
{
    ...
    for (int i = 0; i < VIn.materialCount(); i++)
        matMap[VIn.material(i)] = addMaterial(*(VIn.material(i)));
...
bool CVoxelyze::readJSON(rapidjson::Value &vxl)
{
    ...
    if (!vxl.HasMember("voxelSize") || !vxl["voxelSize"].IsDouble())
    {
        return false;
    }
    voxSize = vxl["voxelSize"].GetDouble();

    if (!vxl.HasMember("materials") || !vxl["materials"].IsArray())
    {
        return false;
    }
    ...
    if (vxl.HasMember("voxels") && vxl["voxels"].IsArray() && vxl["voxels"].Size() % 4 == 0)
    {
        ...
        for (int i = 0; i < 3; i++)
            links[i].resize(maxX - minX + 1, maxY - minY + 1, maxZ - minZ + 1, minX - 1, minY - 1, minZ - 1);

        // add 'em!
        for (int i = 0; i < (int)v.Size() / 4; i++)
            addVoxel(voxelMats[v[i * 4 + 3].GetInt()], v[4 * i].GetInt(), v[4 * i + 1].GetInt(), v[4 * i + 2].GetInt());
    }
    ...
    if (vxl.HasMember("externals") && vxl["externals"].IsArray())
    {
        for (int i = 0; i < (int)vxl["externals"].Size(); i++)
        {
            ...
            if (!(ext.HasMember("voxelIndices") && ext["voxelIndices"].IsArray()))
                continue; // invalid external

            bool dof[6] = {false};
            double disp[6] = {0};
            Vec3D<float> force, moment;

            if (ext.HasMember("fixed") && ext["fixed"].IsArray() && ext["fixed"].Size() == 6)
                for (int j = 0; j < 6; j++)
                {
                    dof[j] = ext["fixed"][j].GetBool();
                }
            if (ext.HasMember("translate") && ext["translate"].IsArray() && ext["translate"].Size() == 3)
                for (int j = 0; j < 3; j++)
                {
                    disp[j] = ext["translate"][j].GetDouble();
                }
            if (ext.HasMember("rotate") && ext["rotate"].IsArray() && ext["rotate"].Size() == 3)
                for (int j = 0; j < 3; j++)
                {
                    disp[3 + j] = ext["rotate"][j].GetDouble();
                }
            if (ext.HasMember("force") && ext["force"].IsArray() && ext["force"].Size() == 3)
                for (int j = 0; j < 3; j++)
                {
                    force[j] = (float)ext["force"][j].GetDouble();
                }
            if (ext.HasMember("moment") && ext["moment"].IsArray() && ext["moment"].Size() == 3)
                for (int j = 0; j < 3; j++)
                {
                    moment[j] = (float)ext["moment"][j].GetDouble();
                }
            ...
            for (int j = 0; j < (int)ext["voxelIndices"].Size(); j++)
            {
                ...
                for (int k = 0; k < 6; k++)
                    if (dof[k])
                        pE->setDisplacement((dofComponent)(1 << k), disp[k]); // fixed degree of freedom
                pE->addForce(force);
                pE->addMoment(moment);
            }
        }
...
bool CVoxelyze::writeJSON(rapidjson::PrettyWriter<rapidjson::StringBuffer> &w)
{
    ...
    if (ambientTemp != 0)
    {
        w.Key("relativeAmbientTemperature");
        w.Double((double)ambientTemp);
    }
    if (grav != 0)
    {
        w.Key("gravityAcceleration");
        w.Double((double)grav);
    }
    if (floor == true)
    {
        w.Key("floorEnabled");
    ...
    { // array of x0, y0, z0, material0, x1, y1, z1, material1, ...
    ...
    if (exts.size() > 0)
    {
        ...
        for (int i = 0; i < (int)exts.size(); i++)
        {
            ...
            if (e->isFixedAny())
            { // if anything is fixed
                w.Key("fixed");
                w.StartArray();
                w.Bool(e->isFixed(X_TRANSLATE));
                w.Bool(e->isFixed(Y_TRANSLATE));
                w.Bool(e->isFixed(Z_TRANSLATE));
                w.Bool(e->isFixed(X_ROTATE));
                w.Bool(e->isFixed(Y_ROTATE));
                w.Bool(e->isFixed(Z_ROTATE));
                w.EndArray();
            }

            if (e->isFixedAnyTranslation() && e->translation() != Vec3D<double>())
            {
                w.Key("translate");
                w.StartArray();
                for (int j = 0; j < 3; j++)
                    w.Double(e->translation()[j]);
                w.EndArray();
            }
            if (e->isFixedAnyRotation() && e->rotation() != Vec3D<double>())
            {
                w.Key("rotate");
                w.StartArray();
                for (int j = 0; j < 3; j++)
                    w.Double(e->rotation()[j]);
                w.EndArray();
            }
            if (!e->isFixedAllTranslation() && e->force() != Vec3D<float>())
            {
                w.Key("force");
                w.StartArray();
                for (int j = 0; j < 3; j++)
            ...
            if (!e->isFixedAllRotation() && e->moment() != Vec3D<float>())
            {
                w.Key("moment");
                w.StartArray();
                for (int j = 0; j < 3; j++)
            ...
            w.Key("voxelIndices");
            w.StartArray();
            for (int j = 0; j < (int)extVoxIndices[i].size(); j++)
                w.Uint(extVoxIndices[i][j]);
            w.EndArray();
            w.EndObject();
        }
        w.EndArray();
...
#ifdef USE_OMP
#pragma omp parallel for shared(Diverged, linksList)
#endif
...
#ifdef USE_OMP
#pragma omp parallel for shared(voxelsList)
#endif
...
{
    // find the largest natural frequency (sqrt(k/m)) that anything in the simulation will experience, then multiply by
    // 2*pi and invert to get the optimally largest timestep that should retain stability
    float MaxFreq2 = 0.0f; // maximum frequency in the simulation in rad/sec

    for (std::vector<CVX_Link *>::const_iterator it = linksList.begin(); it != linksList.end(); it++)
    { // for each link
        CVX_Link *pL = (*it);
        // axial
    ...
    if (MaxFreq2 <= 0.0f)
    { // didn't find anything (i.e no links) check for individual voxelss
        for (std::vector<CVX_Voxel *>::const_iterator it = voxelsList.begin(); it != voxelsList.end(); it++)
        { // for each link
    ...
    if (MaxFreq2 <= 0.0f)
        ...
        return 1.0f /
               (6.283185f *
                sqrt(MaxFreq2)); // the optimal timestep is to advance one radian of the highest natural frequency
}
...
bool CVoxelyze::removeMaterial(CVX_Material *toRemove)
{
    ...
    for (int i = 0; i < materialCount(); i++)
        if (voxelMats[i] == pMat)
            voxelMats.erase(voxelMats.begin() + i);
...
{
    if (!exists((CVX_MaterialVoxel *)replaceMe) || !exists((CVX_MaterialVoxel *)replaceWith))
        return false;
    ...
    for (int iz = indexMinZ(); iz <= indexMaxZ(); iz++)
    {
        for (int iy = indexMinY(); iy <= indexMaxY(); iy++)
        {
            for (int ix = indexMinX(); ix <= indexMaxX(); ix++)
            {
                ...
                if (pV->material() == (CVX_MaterialVoxel *)replaceMe)
                    setVoxel(replaceWith, ix, iy, iz);
            }
        }
    }
    return true;
...
CVX_Voxel *CVoxelyze::addVoxel(CVX_MaterialVoxel *newVoxelMaterial, int xIndex, int yIndex,
                               int zIndex) // creates a new voxel if there isn't one here. Otherwise
{
    try
    {
        ...
        pV->pos = Vec3D<double>(xIndex * voxSize, yIndex * voxSize,
                                zIndex * voxSize); // set initial voxel location (extrapolate?)
...
{
    return links[CVX_Voxel::toAxis(direction)](xIndex + xIndexLinkOffset(direction),
...
CVX_Link *CVoxelyze::addLink(int xIndex, int yIndex, int zIndex, CVX_Voxel::linkDirection direction)
{
    ...
    {
        return pL;
    } // if a link already exists... well, then it should be up to date.

    // ensure that there are voxels at both ends of the link
    CVX_Voxel *voxel1 = voxels(xIndex, yIndex, zIndex);
    CVX_Voxel *voxel2 = voxels(xIndex + xIndexVoxelOffset(direction), yIndex + yIndexVoxelOffset(direction),
                               zIndex + zIndexVoxelOffset(direction));
    if (voxel1 == NULL || voxel2 == NULL)
        return NULL; // if no voxel at either position, don't make a link
    ...
    try
    {
        ...
        links[CVX_Voxel::toAxis(direction)].addValue(xIndex + xIndexLinkOffset(direction),
                                                     yIndex + yIndexLinkOffset(direction),
...
void CVoxelyze::removeLink(int xIndex, int yIndex, int zIndex, CVX_Voxel::linkDirection direction)
{
    CVX_Link *pL = link(xIndex, yIndex, zIndex, direction);
    if (pL == NULL)
        return; // no link here to see!

    // remove the reference in the appropriate link 3d array
    links[CVX_Voxel::toAxis(direction)].removeValue(xIndex + xIndexLinkOffset(direction),
                                                    yIndex + yIndexLinkOffset(direction),
                                                    zIndex + zIndexLinkOffset(direction));
    ...
    CVX_Voxel *voxel1 = voxels(xIndex, yIndex, zIndex);
    if (voxel1)
        voxel1->removeLinkInfo(direction);

    // remove the reference to this link from the other voxel (if it exists)
    CVX_Voxel *voxel2 = voxels(xIndex + xIndexVoxelOffset(direction), yIndex + yIndexVoxelOffset(direction),
                               zIndex + zIndexVoxelOffset(direction));
    if (voxel2)
        voxel2->removeLinkInfo(CVX_Voxel::toOpposite(direction));
...
void CVoxelyze::setAmbientTemperature(float temperature, bool allVoxels)
{
    ...
    if (allVoxels)
    {
        int size = voxelsList.size();
#ifdef USE_OMP
#pragma omp parallel for shared(voxelsList, temperature, size)
#endif
        for (int idx = 0; idx < size; idx++)
        {
            voxelsList[idx]->setTemperature(temperature);
        }
    }
}
...
void CVoxelyze::enableDrag(bool enabled)
{
    drag = enabled;
    for (std::vector<CVX_Voxel *>::iterator it = voxelsList.begin(); it != voxelsList.end(); it++)
    {
        (*it)->enableDrag(enabled);
    }
}
...
void CVoxelyze::updateCollisions()
{
    ...
    float recalcDist =
        (float)(voxSize * watchDistance / 2); // if the voxel moves further than this radius, recalc! //1/2 the
                                              // allowabl, accounting for 0.5x radius of the voxel iself

    // if voxels have been added/removed, regenerate everybody's nearby list
    if (nearbyStale)
    {
#ifdef USE_OMP
#pragma omp parallel for shared(voxelsList, nearbyStale, collisionsStale)
#endif
        for (int i = 0; i < voxelsList.size(); i++)
        {
            voxelsList[i]->generateNearby(watchRadiusVx * 2, false);
        }
        nearbyStale = false;
        collisionsStale = true;
    }
...
#ifdef USE_OMP
#pragma omp parallel for shared(collisionsList)
#endif
        ...
        *pV1->lastColWatchPosition = (Vec3D<float>)pV1->pos; // remember where collisions were last calculated at
        ...
        for (std::vector<CVX_Voxel *>::iterator jt = it + 1; jt != voxelsList.end(); jt++)
        {
            ...
            if (pV2->isInterior() ||                                // don't care about interior voxels here.
                ...
                std::find(pV1->nearby->begin(), pV1->nearby->end(), pV2) !=
                    pV1->nearby->end()) // discard if in the connected lattice array
                continue;
...
float CVoxelyze::stateInfo(stateInfoType info, valueType type)
{
    ...
    if (info == STRAIN_ENERGY || info == ENG_STRESS || info == ENG_STRAIN)
    { // Link properties
        if (linkCount() == 0)
            return 0.0;
        for (std::vector<CVX_Link *>::const_iterator it = linksList.begin(); it != linksList.end(); it++)
        { // for each link
            ...
            {
            case STRAIN_ENERGY:
                thisVal = (*it)->strainEnergy();
                break;
            case ENG_STRESS:
                thisVal = (*it)->axialStress();
                break;
            case ENG_STRAIN:
                thisVal = (*it)->axialStrain();
                break;
            default:
                thisVal = 0;
            }
    ...
    { // voxel properties: DISPLACEMENT, VELOCITY, KINETIC_ENERGY, ANGULAR_DISPLACEMENT, ANGULAR_VELOCITY, PRESSURE,
      // MASS
        if (voxelCount() == 0)
            return 0.0;
        for (std::vector<CVX_Voxel *>::const_iterator it = voxelsList.begin(); it != voxelsList.end(); it++)
        { // for each voxel
            ...
            {
            case DISPLACEMENT:
                thisVal = (*it)->displacementMagnitude();
                break;
            case VELOCITY:
                thisVal = (*it)->velocityMagnitude();
                break;
            case KINETIC_ENERGY:
                thisVal = (*it)->kineticEnergy();
                break;
            case ANGULAR_DISPLACEMENT:
                thisVal = (*it)->angularDisplacementMagnitude();
                break;
            case ANGULAR_VELOCITY:
                thisVal = (*it)->angularVelocityMagnitude();
                break;
            case PRESSURE:
                thisVal = (*it)->pressure();
                break;
            case MASS:
                thisVal = (*it)->material()->mass();
                break;
            default:
                thisVal = 0;
            }
