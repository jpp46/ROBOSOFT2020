...
void CVX_MeshRender::generateMesh()
{
    ...
    for (int k = 0; k < vCount; k++)
    {
        ...
        for (int i = 0; i < 6; i++)
        { // for each direction that a quad face could exist
            if (pV->adjacentVoxel((CVX_Voxel::linkDirection)i))
                continue;
            for (int j = 0; j < 4; j++)
            { // for each corner of the (exposed) face in this direction
                CVX_Voxel::voxelCorner thisCorner = CwLookup[i][j];
                Index3D thisVertInd3D = thisVox + Index3D(thisCorner & (1 << 2) ? 1 : 0, thisCorner & (1 << 1) ? 1 : 0,
                                                          thisCorner & (1 << 0) ? 1 : 0);
                ...
                if (thisInd == -1)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        vertices.push_back(0); // will be set on first updateMesh()
                    }
                    vertexColors.push_back(pV->material()->red() / 255.0f);
                    vertexColors.push_back(pV->material()->green() / 255.0f);
                    vertexColors.push_back(pV->material()->blue() / 255.0f);
                    vertexColors.push_back(pV->material()->alpha() / 255.0f);
                }
    ...
    for (int z = minZ; z < minZ + sizeZ + 1; z++)
    { // for each in vIndMap, now.
        for (int y = minY; y < minY + sizeY + 1; y++)
        {
            for (int x = minX; x < minX + sizeX + 1; x++)
            {
                ...
                // backwards links
                for (int i = 0; i < 8; i++)
                { // check all 8 possible voxels that could be connected...
                    CVX_Voxel *pV =
                        vx->voxel(x - (i & (1 << 2) ? 1 : 0), y - (i & (1 << 1) ? 1 : 0), z - (i & (1 << 0) ? 1 : 0));
                    if (pV)
                        vertexLinks[8 * thisInd + i] = pV;
                }

                // lines
                for (int i = 0; i < 3; i++)
                { // look in positive x, y, and z directions
                    int isX = (i == 0 ? 1 : 0), isY = (i == 1 ? 1 : 0), isZ = (i == 2 ? 1 : 0);
                    int p2Ind = vIndMap[Index3D(x + isX, y + isY, z + isZ)];
                    if (p2Ind != -1)
                    { // for x: voxel(x,y,z) (x,y-1,z) (x,y-1,z-1) (x,y,z-1) -- y: voxel(x,y,z) (x-1,y,z) (x-1,y,z-1)
                      // (x,y,z-1) -- z: voxel(x,y,z) (x,y-1,z) (x-1,y-1,z) (x-1,y,z)
                        if (vx->voxel(x, y, z) || vx->voxel(x - isY, y - isX - isZ, z) ||
                            vx->voxel(x - isY - isZ, y - isX - isZ, z - isX - isY) ||
                            vx->voxel(x - isZ, y, z - isX - isY))
                        {

                            lines.push_back(thisInd);
                            lines.push_back(p2Ind);
                        }
                    }
                }
            }
...
void CVX_MeshRender::updateMesh(viewColoring colorScheme, CVoxelyze::stateInfoType stateType)
{
    ...
    { // for each vertex...
        ...
        /*vertexColors[4 * i] = avgPos.x;
        vertexColors[4 * i + 1] = avgPos.y;
        vertexColors[4 * i + 2] = avgPos.z;
        vertexColors[4 * i + 3] = avgPos.z;*/
    }
    ...
    if (colorScheme == STATE_INFO)
    {
        ...
        if (stateType == CVoxelyze::PRESSURE)
        { // pressure max and min are equal pos/neg
    ...
    for (int i = 0; i < qCount; i++)
    {
        Vec3D<float> v[4];
        for (int j = 0; j < 4; j++)
            v[j] = Vec3D<float>(vertices[3 * quads[4 * i + j]], vertices[3 * quads[4 * i + j] + 1],
                                vertices[3 * quads[4 * i + j] + 2]);
        ...
        case MATERIAL:
            ...
            break;
        case FAILURE:
            if (vx->voxel(quadVoxIndices[i])->isFailed())
            {
                g = 0.0f;
                b = 0.0f;
            }
            else if (vx->voxel(quadVoxIndices[i])->isYielded())
            {
                b = 0.0f;
            }
            break;
        case STATE_INFO:
            switch (stateType)
            {
            case CVoxelyze::KINETIC_ENERGY:
                jetValue = vx->voxel(quadVoxIndices[i])->kineticEnergy() / maxVal;
                break;
            case CVoxelyze::STRAIN_ENERGY:
            case CVoxelyze::ENG_STRAIN:
            case CVoxelyze::ENG_STRESS:
                jetValue = linkMaxColorValue(vx->voxel(quadVoxIndices[i]), stateType) / maxVal;
                break;
            case CVoxelyze::DISPLACEMENT:
                jetValue = vx->voxel(quadVoxIndices[i])->displacementMagnitude() / maxVal;
                break;
            case CVoxelyze::PRESSURE:
                jetValue = 0.5 - vx->voxel(quadVoxIndices[i])->pressure() / (2 * maxVal);
                break;
            default:
                jetValue = 0;
            }
...
float CVX_MeshRender::linkMaxColorValue(CVX_Voxel *pV, CVoxelyze::stateInfoType coloring)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        if (pL)
        {
            ...
            {
            case CVoxelyze::STRAIN_ENERGY:
                thisVal = pL->strainEnergy();
                break;
            case CVoxelyze::ENG_STRESS:
                thisVal = pL->axialStress();
                break;
            case CVoxelyze::ENG_STRAIN:
                thisVal = pL->axialStrain();
                break;
            default:
                thisVal = 0;
            }
...
void CVX_MeshRender::saveObj(std::string fileName)
{
    std::ofstream ofile(fileName + ".obj");
    std::ofstream cfile(fileName + ".clr");
    ofile << "# OBJ file generated by Voxelyze\n";
    for (int i = 0; i < (int)(vertices.size() / 3); i++)
    {
        ofile << "v " << vertices[3 * i] << " " << vertices[3 * i + 1] << " " << vertices[3 * i + 2] << "\n";
        cfile << vertexColors[4 * i] << " " << vertexColors[4 * i + 1] << vertexColors[4 * i + 2] << " "
              << vertexColors[4 * i + 3] << "\n";
    }
    cfile.close();
    ...
    {
        ofile << "f " << quads[4 * i] + 1 << " " << quads[4 * i + 1] + 1 << " " << quads[4 * i + 2] + 1 << " "
              << quads[4 * i + 3] + 1 << "\n";
    }
    ofile.close();
}

void CVX_MeshRender::savePntCld(std::string fileName)
{
    std::ofstream vfile(fileName + ".csv");
    std::ofstream cfile(fileName + ".clr");
    int vCount = vx->voxelCount();
    for (int i = 0; i < vCount; i++)
    {
        ...
        auto mat = pV->material();
        cfile << mat->red() / 255.0f << '\t' << mat->green() / 255.0f << '\t' << mat->blue() / 255.0f << '\t'
              << mat->alpha() / 255.0f << '\n';
    }
    vfile.close();
    cfile.close();
